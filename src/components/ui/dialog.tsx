...
>(({ className, children, showClose = true, closeLabel = 'Close', onOverlayPointerDown, ...props }, ref) => (
  <DialogPortal>
    ...
    >
      {children}
      {showClose && (
        <DialogPrimitive.Close className="touch-target-44 absolute right-3 top-3 flex h-11 w-11 items-center justify-center rounded-md opacity-70 ring-offset-background transition-opacity hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:pointer-events-none md:right-4 md:top-4 md:h-auto md:w-auto">
          <X className="h-4 w-4" />
